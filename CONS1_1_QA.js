...
check('release version remains compatible after CONS-1.1', /^(?:3\.19\.(?:19|2[0-9]|3[01])|3\.20\.[0-7])$/.test(version));
...
check('record schema is versioned', ['1.2.0','1.3.0','1.4.0'].includes(records.VERSION) && records.SCHEMA_VERSION === '1.0');
...
check('assessment form includes consultation record correlation field', assessmentHtml.includes('name="consultation_record_id" id="consultationRecordId"'));
...
check('Agent Workspace exposes record status through a live region', workspaceHtml.includes('id="consultationRecordMeta" role="status" aria-live="polite"'));
