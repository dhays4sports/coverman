...
check('release version remains compatible after DOC-1.1', ['3.19.29','3.19.31','3.20.0','3.20.1','3.20.2','3.20.4','3.20.5','3.20.6','3.20.7'].includes(version));
...
check('three deterministic document page labels are present', executiveSectionSource.includes('Page 1 of 3') && propertySectionSource.includes('Page 2 of 3') && guideSectionSource.includes('Page 3 of 3'));
check('consultation compression CSS is active', rendererSource.includes('CONSULTATION_COMPRESSION_CSS') && rendererSource.includes('cf-consultation-guide'));
check('consultation guide contains required working-document columns', ['What was discovered','Question to ask','Recommended direction','Information to confirm'].every(label => guideSectionSource.includes(label)));
check('consultation guide includes decisions and next action', guideSectionSource.includes('Coverage decisions / proposal structure') && guideSectionSource.includes('Open underwriting or information items') && guideSectionSource.includes('Next action'));
...
check('default document includes deterministic page labels', ['Page 1 of 3','Page 2 of 3','Page 3 of 3'].every(label => html.includes(label)));
...
check('generated document includes decisions workspace and next action', html.includes('Coverage decisions / proposal structure') && html.includes('prepare a home and auto bundle comparison'));
check('generated document uses consolidated consultation guide', html.includes('Coverage Conversation Guide'));
