(function () {
  ...
  const readyPromise = (async () => {
    ...
    function showReport(access) {
      ...
      if (accessStatus) {
        if (access?.localOnly) accessStatus.textContent = expiry ? `Private device-only copy · available until ${expiry}` : 'Private device-only copy';
    ...
    if (reportId && accessApi?.retrieve) {
      ...
      try { allowLegacyPreview = new URLSearchParams(String(location.hash || '').replace(/^#/, '')).get('local_preview') === '1'; } catch (_) {}
      const legacy = allowLegacyPreview ? readLegacyReport() : {};
      if (legacy && Object.keys(legacy).length) {
        ...
        accessResult = { ok: false, code: 'report_unavailable' };
      }
    }
