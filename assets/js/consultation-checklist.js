(function (root, factory) {
  ...
  function applyStoredState(checklist, record, options) {
  ...
  function restoreFromPlan(plan, options) {
    ...
    const storage = resolveStorage(options);
    if (!storage) {
      generated.persistence = { enabled: false, restored: false, storageKey: key, lastSavedAt: '', reason: 'Storage is unavailable.' };
      currentChecklist = generated;
      emitReady('plan-restored');
      return getSnapshot(generated);
    }
    const result = readStoredRecord(key, options);
    if (!result.record) {
      generated.persistence = { enabled: true, restored: false, storageKey: key, lastSavedAt: '', reason: result.reason };
      currentChecklist = generated;
      emitReady('plan-restored');
      return getSnapshot(generated);
    }
    const validation = validateStoredRecord(result.record, generated, options);
    if (!validation.valid) {
      try { storage.removeItem(key); } catch (_) {}
      generated.persistence = { enabled: true, restored: false, storageKey: key, lastSavedAt: '', reason: validation.reason };
      generated.diagnostics.warnings = normalizeStringArray(generated.diagnostics.warnings.concat(validation.reason));
      currentChecklist = generated;
      emitReady('plan-restored');
      return getSnapshot(generated);
    }
    const restored = applyStoredState(generated, result.record, options);
