...
(async () => {
  check('release version remains compatible after CONS-1.2', /^(?:3\.19\.(?:20|2[1-9]|3[01])|3\.20\.[0-7])$/.test(version));
  ...
  check('Workspace exposes visible consultation-document action', workspaceHtml.includes('id="openConsultationDocument"') && workspaceHtml.includes('Open consultation document'));
  ...
  check('document route contains a titled preview frame', consultationHtml.includes('id="consultationDocumentFrame"') && consultationHtml.includes('title="CoverageFit consultation document preview"'));
  ...
  check('document controller exposes stable public API', documentApi && ['1.0.0','1.1.0'].includes(documentApi.VERSION) && typeof documentApi.renderDocument === 'function');
  ...
  check('generated output uses certified report shell with compressed default', rendered.output.diagnostics.reportShellValid === true && !rendered.output.html.includes('data-print-shell="cover"') && rendered.output.html.includes('Page 1 of 3'));
  ...
  check('generated output identifies agent consultation document', rendered.output.html.includes('Agent Consultation Document') && rendered.output.html.includes('Confidential agent consultation document'));
  ...
  check('normal route initialization produces document output', initializedOutput && initializedOutput.html.includes('Home Protection Consultation'));
