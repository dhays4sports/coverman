...
check('release version is ASMT-1.6', ['3.20.6','3.20.7'].includes(version));
check('package version is ASMT-1.6', ['3.20.6','3.20.7'].includes(JSON.parse(read('package.json')).version));
...
check('guide topic carries evidence label', guide.topics[0].evidenceLabel === 'Needs policy verification');
check('guide topic confirmation list carries evidence prompt', guide.topics[0].confirm.some(item => item.includes('water-loss deductible')));
const renderedGuide = guideSection.render(model);
check('consultation document renders confirmed facts', renderedGuide.html.includes('Confirmed facts'));
check('consultation document renders verification items', renderedGuide.html.includes('Verify against policy'));
check('consultation document renders unresolved questions', renderedGuide.html.includes('Unresolved questions'));
check('consultation document renders evidence guardrail', renderedGuide.html.includes('homeowner-reported assessment responses'));
check('consultation topic renders evidence status', renderedGuide.html.includes('Needs policy verification'));
...
check('workspace contains evidence handoff card', workspaceHtml.includes('id="evidenceHandoffCard"') && workspaceHtml.includes('Confirmed facts') && workspaceHtml.includes('Verify against policy') && workspaceHtml.includes('Unresolved questions'));
...
check('consultation document derives evidence-aware next action', documentSource.includes('evidenceFollowUpCount') && documentSource.includes('Resolve ${evidenceFollowUpCount} evidence follow-up'));
...
check('roadmap identifies bounded next sprint', read('ROADMAP.md').includes('ASMT-1.7 Consultation Evidence Resolution Capture'));
